#!/usr/bin/env python3
"""
Production-Realistic Temperature Test
Uses the EXACT same setup as production app (Responses API + web search).
Tests if temperature parameter is actually supported and measures consistency.
"""
...
# Test configurations matching production
temperatures = [1.0, 0.75, 0.5, 0.25, 0.0]
runs_per_temperature = 3
model = "gpt-5-mini"  # Production model
reasoning_effort = "low"  # Production setting

print("=" * 100)
print("PRODUCTION-REALISTIC TEMPERATURE TEST")
print("=" * 100)
print(f"\nConfiguration (matching production app):")
print(f"  - Model: {model}")
print(f"  - API: OpenAI Responses API")
print(f"  - Web Search: ENABLED (production setting)")
...
print("PHASE 1: Testing if Responses API accepts temperature parameter")
print("-" * 100)
...
for temp in temperatures:
    ...
    try:
        # This is EXACTLY how production app calls the API (see components.py:184-195)
        ...
        error_msg = str(e)
        if "temperature" in error_msg.lower() or "unexpected" in error_msg.lower():
...
print("\n" + "=" * 100)
print("PHASE 2: Consistency Testing (if temperature is supported)")
print("=" * 100 + "\n")
...
print("=" * 100)
print("""
This test uses your EXACT production setup:
...
1. Does Responses API accept temperature parameter? (See Phase 1)
...
- If temperature is NOT supported: Consider hybrid caching approach from previous report
""")
print("=" * 100)
