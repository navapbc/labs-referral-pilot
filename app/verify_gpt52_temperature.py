#!/usr/bin/env python3
"""
Test if temperature parameter works with GPT-5.2 + reasoning="none" + web search
Based on OpenAI docs: https://platform.openai.com/docs/guides/latest-model#gpt-5-2-parameter-compatibility
"""
...
print("=" * 80)
print("Testing GPT-5.2 with reasoning='none' for temperature support")
print("=" * 80)
...
if len(accepted_temps) > 1:  # More than just 1.0
    print("✅ SUCCESS! GPT-5.2 with reasoning='none' SUPPORTS custom temperature values!")
    ...
    print("   - temperature=[any value 0.0-1.0]")
    print("\n📊 Next steps:")
    print("   1. Update production config to use GPT-5.2 with reasoning='none'")
    print("   2. Enable temperature parameter in component")
    print("   3. Test with temperature=0.5 or 0.25 for better consistency")
    print("   4. Measure speed impact (GPT-5.2 is ~1.4x slower than 5.1)")
elif len(accepted_temps) == 1 and 1.0 in accepted_temps:
    print("❌ REJECTED: Only temperature=1.0 is accepted (same limitation as GPT-5.1)")
    print("   The OpenAI documentation may be outdated or incomplete.")
else:
    print("❌ FAILED: API rejected all temperature values")
    print("   This contradicts the OpenAI documentation.")
...
output_file = "gpt52_temperature_verification_results.json"
