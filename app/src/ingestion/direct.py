...
def __retrieve_documents(query: str, top_k: int = 10) -> list[Document]:
    ...
    retriever = ChromaEmbeddingRetriever(document_store=config.chroma_document_store())
    text_embedder = SentenceTransformersTextEmbedder(model=config.rag_embedding_model)
...
def __rag_query(query: str, user_email: str) -> dict:
...
def __print_retrieved_docs(docs: list[Document]) -> None:
...
def __export_db_supports_to_md_file() -> None:  # pragma: no cover
...
if __name__ == "__main__":
    ...
    print("Running rag_utils module...")
    logger.info("Running rag_utils module...")
    ...
    if command == "export":
        ...
        query = sys.argv[2]
        response = __rag_query(query, user_email="any@email.com")
