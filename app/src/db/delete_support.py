...
def delete_support_by_name(db_session: Session, support_name: str) -> bool:
    ...
    remaining_supports_count = (
        db_session.query(Support)
        .where(Support.support_listing_id == support_listing_id)
        .count()
    )
...
def main() -> None:  # pragma: no cover
    ...
    parser = argparse.ArgumentParser(
        description="Delete a Support entry by name and its related SupportListing if no other supports remain"
    )
    parser.add_argument("support_name", help="Name of the support to delete")

    args = parser.parse_args()

    with config.db_session() as db_session, db_session.begin():
        success = delete_support_by_name(db_session, args.support_name)
